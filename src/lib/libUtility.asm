...
.macro LIBUTILITY_SET1000_AV(wStart, bValue, wBankOffset)
{
    lda #bValue         // Get value to set
    ldx #250            // Set loop value
loop:
    dex                             // Step -1
    sta wStart+wBankOffset,x        // Set start + x
    sta wStart+wBankOffset+250,x    // Set start + 250 + x
    sta wStart+wBankOffset+500,x    // Set start + 500 + x
    sta wStart+wBankOffset+750,x    // Set start + 750 + x
    bne loop                        // If x != 0 loop
}

//===============================================================================

.macro LIBUTILITY_WAITLOOP_V(wNumLoops)
{
  ldx #<wNumLoops                 // Low byte
  ldy #>wNumLoops                 // High byte
loop:
  dex
  bne loopEnd
  dey
