...
.macro LIBSCREEN_DRAWTEXT_VVA(bXPos, bYPos, string, wBankOffset)
{
    ...
    clc
    adc #>wBankOffset           // add the high byte of wBankOffset
    sta ZeroPage3
...
.macro LIBSCREEN_DRAWTEXT_INDIRECT_VVA(bXPos, bYPos, stringPointerLabel, wBankOffset)
{
    lda #bYPos
    asl
    tay
    ...
    // Add X offset (bXPos) to the screen pointer
    lda ZeroPage2
    clc
    adc #bXPos
    sta ZeroPage2
...
.macro LIBSCREEN_SETBACKGROUND_AA(wBackground, wColor, wBankOffset)
{
...
.macro LIBSCREEN_SET_VIC_BANK_V(bBank)
{
    // Usage:
    //   LIBSCREEN_SET_VIC_BANK_V(<bankbits>)
    //
    //  where <bankbits> must be a value 0..3 that directly matches 
    //  the bits to set in $DD00 (bits 0..1).
    lda     $dd00
    and     #%11111100     // Clear out bits 0 and 1
    ora     #bBank         // Set bits 0..1 to desired bank
    sta     $dd00
}

//==============================================================================

.macro LIBSCREEN_DRAW_N_CHAR_VVVA(bXPos, bYPos, charCode, num, wBankOffset)
{
    lda #bYPos                  // load y position as index into list
    asl                         // X2 as table is in words
    tay                         // Copy A to Y
